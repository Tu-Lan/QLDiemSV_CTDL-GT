...
// Xóa 1 hoc sinh ? v? trí b?t kì(h?y p d?ng sau q)
...
int xoa_HS_ma(ListHS &LHS, int maxoa)
{
  NodeHS *p = LHS.Head, *q = NULL;
  cout << "\nNhap ma can xoa: ";
  cin >> maxoa;
  while (p != NULL)
  {
    if (p->info.MaHS == maxoa)
      break;
    q = p;
    p = p->Next;
  }
...
// 6 xoa
int xoaMon(ListMH &LMH, int ma)
{
  NodeMH *p = LMH.Head, *q = NULL;
  cout << "\nNhap ma mon can xoa:";
  cin >> ma;
  while (p != NULL)
  {
